...
class VideoClsDatasetFrame(Dataset):
    ...
    def __init__(self, anno_path, data_path, mode='train', clip_len=8,
        ...
        self.min_audio_length = 480000
        self.audio_sample_rate = 48000
        self.melbins=64 
        self.frame_length=21
        self.frame_shift=10
    ...
    def __getitem__(self, index):
        if self.mode == 'train':
            ...
            buffer_audio = self.load_audio_for_clap(
                sample_audio, vr, start_frame_idx, end_frame_idx, min_audio_length=self.min_audio_length)
    ...
    def load_video(self, sample, sample_audio, sample_rate_scale=1):
        """Load video content using Decord"""
        fname = sample
        buffer = torch.rand(16, 3, 224, 224)
        if not (os.path.exists(fname)):
            print(f'path not exist {fname}')
            return []
        ...
        if not self.args.audio_only:
            buffer = vr.load(all_index)
            assert len(buffer) == self.clip_len, f"Error: wrong number of frames loaded: {len(buffer)} (expected {self.clip_len})!"
    ...
    def load_audio_for_clap(self, audio_name, video_reader, start_frame_idx, end_frame_idx, min_audio_length):
        ...
        # audio_waveform, sample_rate = torchaudio.load(audio_name)
        if os.path.exists(audio_name):
            audio_waveform, sample_rate = torchaudio.load(audio_name)
        else:
            duration_sec = audio_end - audio_start
            num_samples = int(duration_sec * self.audio_sample_rate)
            audio_waveform = torch.zeros(1, num_samples)  # 单通道，填0
        ...
        audio_start_idx = int(audio_start * audio_waveform.shape[1])
        audio_end_idx = int(audio_end * audio_waveform.shape[1])
        if (audio_end_idx - audio_start_idx) >= min_audio_length:
            audio_waveform = audio_waveform[:, audio_start_idx:audio_end_idx]
        else:
            audio_waveform = audio_waveform.repeat(1, int(min_audio_length / audio_waveform.shape[1]))
            # 补0
            audio_waveform = torch.cat((audio_waveform, torch.zeros(1, min_audio_length - audio_waveform.shape[1])), dim=1)
            
        fbank, _ = self._wav2fbank(audio_waveform, sr=sample_rate)
    ...
    def _wav2fbank(self, waveform1, waveform2=None, sr=None):
        ...
        # 498 128, 998, 128
        # if self.args.load_audio_for_clap:
        fbank = torchaudio.compliance.kaldi.fbank(waveform, htk_compat=True, 
                                                  sample_frequency=sr, 
                                                  use_energy=False,
                                                window_type='hanning', 
                                                num_mel_bins=self.melbins, dither=0.0, 
...
def compute_mel_spec_audiomae(audio_tensor, 
    ...
    mel_spec = mel_spec[0].permute(1, 0) # freq, time
    return mel_spec
