import models
import warnings
from utils.plot_logs import plt_logs
import utils as utils
from utils import multiple_samples_collate
from utils import NativeScalerWithGradNormCount as NativeScaler
from engine import train_one_epoch, validation_one_epoch, final_test, merge
from datasets.datasets_av import build_dataset
import torch.multiprocessing as mp
from utils.optim_factory import create_optimizer, get_parameter_groups, LayerDecayValueAssigner, LayerDecayValueAssignerForAudioVisual
...
def get_args():
    parser = argparse.ArgumentParser(
        'HiCMAE fine-tuning and evaluation script for audio-visual emotion recognition', add_help=False)
    ...
    # 添加新参数
    parser.add_argument('--prompt_lr_scale', type=float, default=1.0,
                        help='learning rate scale for prompt learner parameters')
    ...
    parser.add_argument('--num_audio_frames', type=int, default=1)
    parser.add_argument('--load_audio_for_clap',
                        action='store_false', default=True)

    parser.add_argument('--sampling_rate', type=int, default=4)
    parser.add_argument('--data_set', default='Kinetics-400', choices=['Kinetics-400', 'SSV2', 'UCF101', 'HMDB51', 'image_folder',
                        'DFEW', 'MAFW', 'MAFW_43', 'RAVDESS', 'CREMA-D', 'AVCAFFE', "MER2023", "MSP-IMPROV", "IEMOCAP"],
    ...
    parser.add_argument('--freqm', help='frequency mask max length',
                        type=int, default=0)  # pretraining 0
    parser.add_argument('--timem', help='time mask max length',
                        type=int, default=0)  # pretraining 0
    ...
    parser.add_argument('--min_audio_length', type=int, default=16000)
    parser.add_argument('--early_stop_epoch', type=int, default=20)
    parser.add_argument('--disable_mixup',
                        action='store_true', help='disable mixup')
    parser.set_defaults(disable_mixup=False)
    parser.add_argument(
        '--roll_mag_aug', action='store_false', help='use roll_mag_aug')
    parser.set_defaults(roll_mag_aug=True)

    parser.add_argument('--save_feature', action='store_true', default=False)
    parser.add_argument('--lamda_kl', type=float, default=0.0,
                        help='KL loss weight for audio-visual alignment')
    parser.add_argument('--lamda_en', type=float, default=0.0,
                        help='entropy loss weight for audio-visual alignment')
...
def main(local_rank, nprocs, args, ds_init):
    ...
    if mixup_active and not args.disable_mixup:
    ...
    print('number Number of trainable parameters:', n_parameters)
    ...
    utils.auto_load_model(
        ...
        optimizer=optimizer, loss_scaler=loss_scaler, model_ema=model_ema, load_best=args.eval)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if data_loader_val is not None:
            ...
            if (args.val_metric not in ['loss'] and best_metric < test_stats[args.val_metric]) or \
                ...
                early_stop_counter += 1  # 没提升，计数器+1
                print(
                    f"Early stop counter: {early_stop_counter}/{args.early_stop_epoch}")
                if early_stop_counter >= args.early_stop_epoch:
                    print(f"Early stopping at epoch {epoch}!")

                    break
            print(
                f"Best '{args.val_metric.upper()}': {best_metric:.4f}% UAR: {best_stats['uar']*100:.2f}%, WAR: {best_stats['war']*100:.2f}% @ (epoch={best_epoch})")
            if log_writer is not None:
                log_writer.update(
                    train_acc1=train_stats['class_acc'], head="perf", step=epoch)
                log_writer.update(
                    train_loss=train_stats['loss'], head="perf", step=epoch)
                log_writer.update(
        ...
        if args.output_dir and utils.is_main_process():
            ...
            with open(os.path.join(args.output_dir, "log.txt"), mode="a", encoding="utf-8") as f:
                f.write(json.dumps(log_stats) + "\n")
            plt_logs(args.output_dir)
        remain_time = (time.time() - start_time) / \
            (epoch - args.start_epoch + 1) * (args.epochs - epoch)
    ...
    if True:
        ...
        preds_file = os.path.join(
            args.output_dir, str(global_rank) + '_best.txt')
        ...
        if global_rank == 0:
            ...
            final_top1_best, final_top5_best, pred_dict_best = merge(
                args.output_dir, num_tasks, args, best=True)
            print(
                f"Accuracy of the network on the {len(dataset_test)} test videos using best epoch model: Top-1: {final_top1_best:.2f}%, Top-5: {final_top5_best:.2f}%")
            log_stats_best = {'Final Top-1 (best epoch)': final_top1_best,
                              'Final Top-5 (best epoch)': final_top5_best}
            from sklearn.metrics import confusion_matrix
            preds_best, labels_best = pred_dict_best['pred'], pred_dict_best['label']
            print(f'Total test samples: {len(preds_best)}')
            conf_mat_best = confusion_matrix(
                y_pred=preds_best, y_true=labels_best)
            print(f'Confusion Matrix:\n{conf_mat_best}')
            class_acc_best = conf_mat_best.diagonal() / conf_mat_best.sum(axis=1)
            print(f"Class Accuracies: {[f'{i:.2%}' for i in class_acc_best]}")
            uar_best = np.mean(class_acc_best)
            war_best = conf_mat_best.trace() / conf_mat_best.sum()
            print(f'UAR: {uar_best:.2%}, WAR: {war_best:.2%}')
            weighted_f1_best = f1_score(
                y_pred=preds_best, y_true=labels_best, average='weighted')
            micro_f1_best = f1_score(
                y_pred=preds_best, y_true=labels_best, average='micro')
            macro_f1_best = f1_score(
                y_pred=preds_best, y_true=labels_best, average='macro')
            print(
                f'Weighted F1: {weighted_f1_best:.4f}, micro F1: {micro_f1_best:.4f}, macro F1: {macro_f1_best:.4f}')
            ...
            if args.output_dir and utils.is_main_process():
                with open(os.path.join(args.output_dir, "log.txt"), mode="a", encoding="utf-8") as f:
                    ...
                    f.write(
                        f'Final UAR: {uar_best:.2%}, Final WAR: {war_best:.2%}\n')
                    f.write(f'Final Confusion Matrix:\n{conf_mat_best}\n')
                    f.write(
                        f'Final Class Accuracies: {[f"{i:.2%}" for i in class_acc_best]}\n')
                    f.write(
                        f'Final Weighted F1: {weighted_f1_best:.4f}, Final Micro F1: {micro_f1_best:.4f}, Final Macro F1: {macro_f1_best:.4f}\n')
