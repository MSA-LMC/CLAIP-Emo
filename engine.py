...
def train_one_epoch(model: torch.nn.Module, criterion: torch.nn.Module,
    ...
    for data_iter_step, ((samples, samples_audio), targets, _, _) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        targets_for_acc = targets.clone()

        if mixup_fn is not None:

            samples, targets = mixup_fn(samples, targets)
            samples_audio, _ = mixup_fn(samples_audio, targets)
        ...
        class_acc = (output.max(-1)[-1] ==
                     targets_for_acc).float().mean() * 100
...
def validation_one_epoch(data_loader, model, device):
    ...
    # print('* AV_Acc@1 {AV_top1.global_avg:.2f} V_Acc@1 {V_acc1.global_avg:.2f} A_Acc@1 {A_acc1.global_avg:.2f}'
    #       .format(AV_top1=metric_logger.AV_acc1, V_acc1=metric_logger.V_acc1, A_acc1=metric_logger.A_acc1))
    # del metric_logger.meters['AV_acc1']
    # del metric_logger.meters['V_acc1']
    # del metric_logger.meters['A_acc1']
    return {k: meter.global_avg for k, meter in metric_logger.meters.items()}
...
def final_test(data_loader, model, device, file, save_feature=False):
    ...
    for batch in metric_logger.log_every(data_loader, 10, header):
        ...
        with torch.cuda.amp.autocast():
            ...
            if save_feature:
                output, saved_feature = model(
                    videos, audios, save_feature)
            else:
                output = model(videos, audios)
            loss = criterion(output, target)
        ...
        for i in range(output.size(0)):
            string = "{} {} {} {} {}\n".format(ids[i],
                                               str(output.data[i].cpu(
                                               ).numpy().tolist()),
                                               str(int(
                                                   target[i].cpu().numpy())),
                                               str(int(
                                                   chunk_nb[i].cpu().numpy())),
            ...
            if save_feature:
                if ids[i] not in saved_features:
                    saved_features[ids[i]] = {'chunk_id': [], 'split_id': [],
                                              ...
                                              'feature': [], 'logit': []}
                saved_features[ids[i]]['chunk_id'].append(
                    int(chunk_nb[i].cpu().numpy()))
                saved_features[ids[i]]['split_id'].append(
                    int(split_nb[i].cpu().numpy()))
                saved_features[ids[i]]['feature'].append(
                    saved_feature.data[i].cpu().numpy().tolist())
                saved_features[ids[i]]['logit'].append(
                    output.data[i].cpu().numpy().tolist())
...
def merge(eval_path, num_tasks, args, best=False):
    ...
    if args.data_set != 'AVCAFFE':
        for x in range(num_tasks):
            file = os.path.join(eval_path, str(
                x) + '.txt') if not best else os.path.join(eval_path, str(x) + '_best.txt')
                ...
                for sample_id in saved_features.keys():
                    ...
                    for idx, (chunk_id, split_id) in enumerate(zip(chunk_ids, split_ids)):
                        ...
                        if chunk_split_id not in overall_saved_features[sample_id]['chunk_split_id']:
                            overall_saved_features[sample_id]['chunk_split_id'].append(
                                chunk_split_id)
                            ...
                            overall_saved_features[sample_id]['prob'].append(
                                softmax(saved_features[sample_id]['logit'][idx]))
        ...
        for x in range(num_tasks):
            ...
            for line in lines:
                ...
                chunk_nb = line.split(']')[1].split(' ')[2]
                split_nb = line.split(']')[1].split(' ')[3]
                data = np.fromstring(line.split('[')[1].split(']')[
                                     0], dtype=np.float, sep=',')
                data = softmax(data)
                if not name in dict_pos:
                    dict_pos[name] = []
                if chunk_nb + split_nb in dict_pos[name]:
                    continue
                sample_name = '_'.join(os.path.basename(name).split('_')[
                                       :3])  # e.g., aiim001_task_1_clip_001
                if not sample_name in dict_feats:
                    dict_feats[sample_name] = []
                    dict_pos[sample_name] = []
                dict_feats[sample_name].append(data)
                dict_pos[name].append(chunk_nb + split_nb)
                dict_label[sample_name] = label
    ...
    if args.save_feature:
        ...
        feature_file = os.path.join(eval_path, 'overall_feature.pkl') if not best else os.path.join(
            eval_path, 'overall_feature_best.pkl')
